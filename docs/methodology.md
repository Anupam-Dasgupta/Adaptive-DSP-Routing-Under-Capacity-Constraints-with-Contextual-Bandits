...
Avazu supplies realistic request contexts only. Its `click` label is neither a model
feature nor a DSP reward. Avazu cannot provide counterfactual responses from several
DSPs for the same request, so those responses are generated by the simulator.
