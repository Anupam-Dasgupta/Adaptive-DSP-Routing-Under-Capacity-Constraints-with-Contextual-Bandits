"""Quick abrupt-drift baseline comparison."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--world", choices=("linear", "two-stage"), default="two-stage"
    )
    ...
    policies = {
        ...
        "DiscountedLinUCB": DiscountedLinUCBPolicy(
            args.n_dsps,
            features.shape[1],
            alpha=args.alpha,
            gamma=args.gamma,
    ...
    metrics_path = ROOT / "results" / "metrics" / "drift_quick.json"
    ...
    figure_path = plot_comparison(
        results,
        ROOT / "results" / "figures" / "drift_quick.png",
        drift_time=drift_time,
    )
